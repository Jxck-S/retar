    ...
    if [[ -n $1 ]] && [ "$1" != "test" ] && [ "$1" != "force" ]; then
        ...
        true
    elif [[ -f $RUNBASE/dump1090-fa/aircraft.json ]] ; then
        srcdir=$RUNBASE/dump1090-fa
    elif [[ -f $RUNBASE/readsb/aircraft.json ]]; then
        srcdir=$RUNBASE/readsb
    elif [[ -f $RUNBASE/adsbexchange-feed/aircraft.json ]]; then
        srcdir=$RUNBASE/adsbexchange-feed
    elif [[ -f $RUNBASE/dump1090/aircraft.json ]]; then
        srcdir=$RUNBASE/dump1090
    elif [[ -f $RUNBASE/dump1090-mutability/aircraft.json ]]; then
        srcdir=$RUNBASE/dump1090-mutability
    elif [[ -f $RUNBASE/skyaware978/aircraft.json ]]; then
        srcdir=$RUNBASE/skyaware978
    elif [[ -f $RUNBASE/shm/aircraft.json ]]; then
        srcdir=$RUNBASE/shm
    else
        echo --------------
        echo FATAL: could not find aircraft.json in any of the usual places!
        echo "checked these: $RUNBASE/readsb $RUNBASE/dump1090-fa $RUNBASE/dump1090 $RUNBASE/dump1090-mutability $RUNBASE/adsbexchange-feed $RUNBASE/skyaware978"
        echo --------------
    ...
    if [[ -d /usr/local/share/adsbexchange-978 ]]; then
        instances+="\n $RUNBASE/adsbexchange-978 ax978"
    fi
